    ...
    <Card className="group hover:shadow-lg transition-all duration-200 border-[#E2E8F0] hover:border-[#1B8A44]/20 h-full flex flex-col">
        ...
        {userRole === 'student' && course.is_enrolled && course.progress !== undefined && (
          <div className="absolute bottom-0 left-0 right-0 bg-white/90 p-2">
            <div className="flex items-center gap-2 text-xs">
              <div className="flex-1 bg-[#E2E8F0] rounded-full h-1.5 sm:h-2">
                <div 
                  className="bg-[#1B8A44] h-1.5 sm:h-2 rounded-full transition-all duration-300"
                  style={{ width: `${course.progress}%` }}
                />
              </div>
              <span className="text-[#64748B] font-medium text-xs">{Math.round(course.progress || 0)}%</span>
            </div>
          </div>
        )}
      </div>
      ...
      <CardContent className="p-3 sm:p-4 flex-1 flex flex-col">
        <div className="space-y-2 sm:space-y-3 flex-1">
          ...
          {/* Instructor */}
          <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-[#64748B]">
